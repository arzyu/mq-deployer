...
(async function () {
  ...
  const mqConnection = await amqp.connect(options.uri);
  const channel = await mqConnection.createChannel();
  const exchange = options.exchange;
  const router = options.router;
  const taskQueue = `${exchange}:${router}:${uuid.v4()}`;
  ...
  channel.consume(taskQueue, (message) => {
    ...
    (async function () {
      ...
      await reply({ state: 'DONE', message: '<- [consumer]: Task completed.' });
      ...
      if (e.name === 'SyntaxError' && e.message.includes('JSON')) {
        ...
        (async function () {
          ...
          await reply({
            state: 'REFUSED',
            message: `<- [consumer]: Invalid JSON message: ${messageText}, dropped.`
          });
        ...
        (async function () {
          ...
          await reply({
            state: 'ACCEPTED',
            message: `<- [consumer]: Consumer error: ${e.stack}`
          });
        }()).catch(console.error);
